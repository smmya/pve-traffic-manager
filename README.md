...
机器人只接受 PTM 中配置的单一授权会话；未授权会话除 `/id` 外不能使用任何管理功能。支持：

- `/status`：系统、虚拟机和预警摘要
- `/traffic`、`/group <组ID>`、`/vm <VMID> [qemu|lxc]`：流量查询
- `/alerts`、`/shutdowns`、`/logs`：预警、超限关机和操作记录
- `/collect`：立即采集一次流量，但不会执行关机
- `/resetvm <VMID> <qemu|lxc>`：二次确认后，仅重置该机器在全部组中的流量
- `/menu`、`/help`：按钮菜单和命令帮助
...
| Telegram 通知 | 默认 80% 单次预警、超限关机通知、失败自动重试 |
| Telegram Bot | 授权会话内查询/采集/日志/单机流量重置，含按钮菜单与二次确认 |
